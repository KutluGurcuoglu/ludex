...
const DEMO_ROLES: { role: UserRole; label: string }[] = [
  { role: "judge", label: "Hakem" },
  { role: "contestant", label: "Yarışmacı" },
...
export default function LoginPage() {
  ...
  function handleGoogleClick() {
    toast.info("Google ile giriş bu demoda bağlı değil.", {
      description: "E-posta ile giriş yapabilir veya aşağıdaki demo hesaplarını kullanabilirsin.",
    });
  }
          ...
          {awaitingVerification ? (
            ...
            <>
              <button
                ...
                className="flex w-full items-center justify-center gap-3 rounded-full bg-white px-4 py-2.5 text-sm font-semibold text-black transition-all hover:bg-gray-100 active:scale-[0.98]"
              >
              ...
              <div className="mt-3">
                ...
                <div className="flex w-full rounded-full border border-border/60 bg-muted p-1">
                  {DEMO_ROLES.map(({ role: demoRole, label }) => (
                    <button
                      key={demoRole}
                      type="button"
                      onClick={() => demoLogin(demoRole)}
                      className="flex-1 rounded-full px-3 py-1.5 text-xs font-medium text-muted-foreground transition-colors hover:bg-background hover:text-foreground active:scale-[0.97]"
                    >
                      {label}
                    </button>
                  ))}
